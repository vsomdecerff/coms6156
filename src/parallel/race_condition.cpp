...
//    MPI_Init(NULL, NULL);
    ...
    if (rank == 0) {
        ...
        MPI_Isend (&a, 1, MPI_UNSIGNED, dest, 0, MPI_COMM_WORLD, &req);
        a = -1;
//        MPI_Wait (&req, &stat);
        ...
        MPI_Recv(&a, 1, MPI_UNSIGNED, source, 0, MPI_COMM_WORLD, &stat);	
    }
...
unsigned int race_condition_2(unsigned int data)
{
    ...
    if (rank == 0) {
        ...
        // Start immediate send
        MPI_Isend (&a, data, MPI_UNSIGNED, dest, 0, MPI_COMM_WORLD, &req);
    }
    else if (rank == 1) {
		for (int i = 0; i < data; i++) {
            a[i] = 0;
        }
        MPI_Irecv(&a, data, MPI_UNSIGNED, source, 0, MPI_COMM_WORLD, &req);
